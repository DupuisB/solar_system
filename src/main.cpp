...
void initGPUgeometry() {
  ...
  // Same for a vertex buffer object that stores the colors of the vertices
  ...
  glBindBuffer(GL_ARRAY_BUFFER, g_colVbo);
  glNamedBufferStorage(g_colVbo, colorBufferSize, g_vertexColors.data(), GL_DYNAMIC_STORAGE_BIT);
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3*sizeof(GLfloat), 0);
  glEnableVertexAttribArray(1);
#endif
